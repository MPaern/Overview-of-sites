...
```{r daytime bats, include=FALSE}

# vector of all the days active in 2024
days <- seq(from = as.POSIXct("2024-06-28"), to = as.POSIXct("2024-10-07"), by = "days")
...
noise$DATE <- as.POSIXct(noise$DATE, format="%Y-%m-%d")
#class(wo_noise$MANUAL.ID)
noise$TIME <- as.character(noise$TIME)
noise$TIME <- hms(noise$TIME)
wo_noise$DATE <- as.POSIXct(wo_noise$DATE, format="%Y-%m-%d")
wo_noise$TIME <- as.character(wo_noise$TIME)
wo_noise$TIME <- hms(wo_noise$TIME)
...
ggplot(sun_times, aes(x = date, ymin = sunrise, ymax = sunset)) +
  labs(
    title = "Plot 2. 2024 Recorded Sounds Compared to Daylight",
    subtitle = "Vistenes, Norge"
  ) +
  ...
  geom_point(
    data = noise,
    aes(x = DATE, y = TIME, fill = MANUAL.ID),
    inherit.aes = FALSE
  ) +
  geom_point(
    data = wo_noise,
    inherit.aes = FALSE,
    aes(x = DATE, y = TIME, colour = MANUAL.ID),
  ) +
  scale_colour_manual(
    values = c(
      "VESMUR" = "blue", 
